...
import random
import cv2
from model import create_siamese_net  # Import create_siamese_net from model.py
...
if __name__ == "__main__":
    ...
    for i in range(0, 200000):
        ...
        loss = siamese_net.train_on_batch(inputs, targets)  # Changed SiameseNet to siamese_net
        loss_history.append(loss)
        if i % loss_every == 0:
            vloss = siamese_net.test_on_batch(*getMiniBatch(batch_size, path="eval"))  # Changed SiameseNet to siamese_net
            print("iteration {}, training loss: {:.7f}, validation loss : {:.7f}".format(i, np.mean(loss_history), vloss))
            loss_history.clear()
            val_acc = test_oneshot(siamese_net, N, verbose=True)  # Changed SiameseNet to siamese_net
            if val_acc >= best:
                print("saving")
                # Save the weights with .h5 extension
                siamese_net.save("saved_best.h5")
                # Save weights with .h5 extension
                best = val_acc
